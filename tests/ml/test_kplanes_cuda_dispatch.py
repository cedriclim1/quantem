...
from quantem.tomography.dataset_models import TomographyEDSINRDataset
...
@requires_gpu
@requires_quantem_cuda
def test_multichannel_eds_model_forces_torch_path_and_integrates_rays(monkeypatch):
    import quantem.cuda.core.ml

    def boom(pts, rotations, plane):
        raise AssertionError("multi-channel heads must not call the CUDA kernel")

    monkeypatch.setattr(quantem.cuda.core.ml, "kplanes_tilted_fuse", boom)
    batch, samples, channels = 3, 5, 5
    model = KPlanesTILTED(
        M_features=2,
        resolution=(8, 8, 8),
    ...
    coords = torch.rand(batch * samples, 3, device="cuda") * 2 - 1

    densities = model(coords)
    integrated = TomographyEDSINRDataset._integrate_multichannel_rays(
        densities, samples, batch
    )
    expected = densities.view(batch, samples, channels).sum(dim=1) * (2.0 / (samples - 1))

    assert integrated.shape == (batch, channels)
